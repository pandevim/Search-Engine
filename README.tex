...
\begin{enumerate}
    ...
    \item \textbf{Stemming/Lemmatization}: Reduces words to their root form (e.g., "running" $\rightarrow$ "run") using a dictionary-based approach sourced from \texttt{data/lemmatization-en.txt} (\href{https://github.com/michmech/lemmatization-lists}{Source}).
\end{enumerate}

\subsection{3. Indexer}
...
\begin{itemize}
    \item \textbf{Interactive CLI}: A command-line loop that accepts user queries.
    ...
    \item \textbf{Result Display}: Shows the top 10 matching document paths, titles, and scores.
\end{itemize}

\subsection{5. Server (REST API)}
...
\subsection{1. Running the Crawler}
...
\subsection{2. Running the Indexer}
...
\subsection{3. Running the Search}
...
\subsection{4. Running the Server}
...
\subsection{5. Running the Client}
...
The stemming algorithm implementation is adapted from \href{https://github.com/minhnhdo/rust-stem}{rust-stem}.
